...
class QSystem:
    ...
    def __init__(self, num_qubits, lock, index = None, state = None):
        '''
    ...
    def from_stream(cls, qstream, index):
        ...
        '''
        return cls(qstream.system_size, qstream.lock, index = index, state = qstream.state[index])
    ...
    def measure_qubit(self, index):
        ...
        try:
            ...
            if np.random.default_rng().random() <= prob0:
